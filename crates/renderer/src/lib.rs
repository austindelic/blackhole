//! Native offscreen black-hole renderer. GPU work can run on a background [`Worker`].
pub mod frame;
pub mod route;
use anyhow::{Context, Result};
pub use frame::{Cell, CellFrame};
pub use glam::Vec3;
    ...
    async fn initialize() -> Result<Self> {
        ...
        let adapter = instance
            .request_adapter(&wgpu::RequestAdapterOptions {
                ...
                force_fallback_adapter: std::env::var_os("BLACKHOLE_SOFTWARE_GPU").is_some()
                    || std::env::var_os("AUSTINDELIC_SOFTWARE_GPU").is_some(),
                ..Default::default()
            })
            .await
...
impl Worker {
    ...
    fn start_with_renderer(
        ...
        let thread = std::thread::spawn(move || {
            let result =
                std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| -> Result<()> {
                    ...
                    *msg.lock().unwrap() = format!("GPU · {}", renderer.adapter);
                    ...
                    let interval = Duration::from_secs_f64(1. / f64::from(fps.max(1)));
            ...
            match result {
                Ok(Ok(())) => {}
                Ok(Err(e)) => *msg.lock().unwrap() = format!("Renderer unavailable: {e}"),
                Err(_) => *msg.lock().unwrap() = "Renderer unavailable: GPU worker failed".into(),
            }
        });
        Self {
            request,
    ...
    /// Take the newest completed frame without waiting for the worker.
    pub fn take_frame(&self) -> Option<CellFrame> {
        self.frame.try_lock().ok()?.take()
    }
    /// Read worker status without blocking.
    pub fn status(&self) -> Option<String> {
        self.status.try_lock().ok().map(|s| s.clone())
    }
    /// Replace pending work with the latest request (bounded, latest-wins queue).
        ...
        for panic in [false, true] {
            ...
            assert!(
                worker
                    ...
                    .lock()
                    .unwrap()
                    .starts_with("Renderer unavailable:")
            );
