...
import {
	...
	getBlackHoleRouteAnimation as lookupRouteAnimation,
	normalizeBlackHoleAnimationRoute as normalizeRoute,
...
export type RuntimeState = {
 ...
 persistentAnimationSnapshot?: import("./BlackHoleCore").PersistentAnimationSnapshot;
...
function startRendererSession(
	...
	const routes = state.initialProps.routes ?? BLACK_HOLE_ANIMATION_ROUTES;
 const getBlackHoleRouteAnimation = (path: string | undefined) => lookupRouteAnimation(path, routes);
 const normalizeBlackHoleAnimationRoute = (path: string | undefined) => normalizeRoute(path, routes);
 let exploring = false;
 let heldCameraPath: string | null = null;
	...
	const activeAnimationMode = (): AnimationMode => state.animationMode;
	...
	const currentAnimationRoute = () => {
		const source = state.animationRoute;
	...
	const persistedAnimationSnapshot =
		activeAnimationMode() === "route"
			? state.persistentAnimationSnapshot
			: undefined;
	...
	const snapshotRuntime = (
		...
		if (!state.paused && frame > 0 && !forcePersistent && now - lastRuntimeSnapshotUpdate < 250) return;
		...
		const persistent = state.persistentAnimationSnapshot;
		if (persistent) {
 ...
 const motionReduced = () => reducedMotion.matches && !state.initialProps.forceActiveRender;
	...
	const syncOrbitRoute = () => {
		...
		if (routeIntroPending) {
			...
			if (route === "/" && !motionReduced()) {
				startIntro("/", true);
				return;
			}
		}
		const inIntro = animationPhase === "intro" && !orbitController;
		if (inIntro && route === activeAnimationRoute && !motionReduced())
			return;
	...
	const syncAnimationRoute = () => {
		...
		if (heldCameraPath !== null) {
			if (heldCameraPath === state.animationRoute) return;
			heldCameraPath = null;
	...
	state.animationEditor = {
		...
		routeConfig: () =>
			stringifyAnimationValue(
				getBlackHoleRouteAnimation(activeAnimationRoute),
			),
	};
	...
	const publishStats = (frameTimeMs: number, now: number) => {
		if (!debugStats && !showControls) return;
		if (!state.paused && frame > 0 && !showControls && now - lastStatsPublish < 250) return;
		lastStatsPublish = now;

		const activeControls = exploring || activeAnimationMode() === "off" ? state.controls : lastAnimatedControls;
		...
		const stats: BlackHoleStats = {
			...
			asciiEnabled: exploring ? exploreAsciiEnabled : activeAnimationMode() === "off" ? settings.asciiEnabled : lastAnimatedAsciiEnabled,
	...
	const renderFrame = (now: number) => {
		...
		try {
			...
			const delta = state.paused ? 0 : Math.min(0.1, Math.max(0.001, (now - lastTime) / 1000));
			...
			snapshotRuntime(motionReduced(), now);
			...
			let submitted: boolean | void = undefined;
			try {
		...
		if (!state.paused && !document.hidden && !motionReduced()) {
			animationFrame = requestAnimationFrame(renderFrame);
		}
	};
 ...
 if (pressed && !exploring && document.activeElement !== canvas) return;
 ...
 clearInput();
 movementSpeed = explorationEntry.movementSpeed;
 heldCameraPath = state.animationRoute;
...
export function createRuntimeState(props: Props): RuntimeState {
	const initialProps = {
		...props,
 forceActiveRender: props.forceActiveRender ?? props.paused === false,
		textSize:
...
export function mountBlackHoleRuntime(
	...
	const start = () => {
		...
		cleanup?.();
 if (state.resetCameraRequested) {
 state.runtimeSnapshot = {shaderTime: state.runtimeSnapshot.shaderTime};
 state.persistentAnimationSnapshot = undefined;
 state.resumeAnimation = undefined;
 state.resetCameraRequested = false;
 }
		...
		if (document.fonts.check(font) && !["Departure Mono", "DSEG14Modern"].includes(state.atlasConfig.fontFamily)) mount();
		else {
			...
			void loadFonts(document).then(() => document.fonts.load(font)).then(mount, (error) => {
 if (disposed || generation !== currentGeneration) return;
 options.onError(`Font loading failed; using system fallback: ${formatError(error)}`);
 mount();
 });
		}
	};
