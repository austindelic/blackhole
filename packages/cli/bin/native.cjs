...
function executable(platform, arch, root = path.resolve(__dirname, '..'), resolve = require.resolve) {
  ...
  const name = `${packageScope}blackhole-${target}`;
  try {
    return resolve(`${name}/bin/${platform === 'win32' ? 'blackhole.exe' : 'blackhole'}`, { paths: [root] });
  } catch (error) {
    if (error.code !== 'MODULE_NOT_FOUND') throw error;
    throw new Error(`The native package ${name} is missing. Reinstall blackhole with npm install --include=optional ${packageName}. Do not omit optional dependencies; no runtime download will be attempted.`);
  }
}
function launch({ argv = process.argv.slice(2), host = process, spawnProcess = spawn, exists = existsSync, root, resolve } = {}) {
  ...
  try {
    ...
    if (!exists(file)) throw new Error(`The native executable is missing for ${host.platform}-${host.arch}. Reinstall blackhole; no runtime download will be attempted.`);
  } catch (error) {
    host.stderr.write(`blackhole: ${error.message}\n`);
    host.exitCode = 1;
    return;
  }
  ...
  child.once('error', (error) => {
    cleanup();
    host.stderr.write(`blackhole: unable to start the native application: ${error.message}\n`);
    host.exitCode = 1;
  });
